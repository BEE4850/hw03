...
Suppose that, after conducting the procedure on your class of 100 students, you get 28 "Yes, I cheated" responses. You are skeptical that your class is broadly engaged in cheating and have a prior belief that the probability of cheating $\theta$ is unlikely to be high. We'll express this prior using a beta distribution, $\theta \sim \Beta(3, 80)$. You would like to understand the range of cheating propensity which is consistent with your prior beliefs and the outcomes from the detection procedure.


**In this problem**:

- Construct a Bayesian probability model (based on the data-generating process from Homework 1 and our prior on the cheating probability) for the "Yes, I cheated" counts. You can compute the likelihood based on the combination of the fair coin flip and the "true" cheating probability $\theta$. How did you decide that you had conducted enough simulations for your outcome distributions to be "stable"?
...
- Repeat the analysis with a less constrained prior of your choosing. How does that change your inferences and conclusions?
...
You have been asked by a client to assess the risks of flooding to their home (which is valued at \$400,000 and only floods when the stream water levels exceed 1m). They would like to know the probability distribution of annual flood damages to their home if they do not take any preventative floodproofing measures. After some hydrologic analysis, you have developed a 40-year record of annual maximum water levels (in m) at the nearby stream. 
...
where $k=0.75$ and $x_0=2.5$^[As a reminder, the indicator function $\mathbb{I}[h > 0]$ is $0$ when the condition is not satisfied (in this case, when $h=0$) and $1$ when it is satisfied.].  The graph of this function is given in the figure below.
...
plot(h, dd(h, 0.75, 2.5), xlabel="Flood Depth (m Above Base Elevation)", ylabel="Fraction of Value Damaged", legend=false)
ylims!(0, 1)
```
...
For this problem, we will assume that the cost of elevating the house $\Delta h$ m is $C(\Delta h) = \mathbb{I}[h > 0](100,000 + 3,000\Delta h)$. Calculating the NPV of the benefits of elevating by $\Delta h$ m requires computing how the elevation changes the expected flooding damages over the design horizon and comparing these benefits to the elevation cost.


**In this problem**:

- Using the extreme water level distribution from Problem 2, use Monte Carlo simulation to estimate the NPV of expected flooding for elevation levels between 1 and 5m (you don't have to consider increments finer than 0.5m). What elevation heights pass the cost-benefit test?^[This is an example of **Monte Carlo optimization** as you are using Monte Carlo simulation to estimate the optimization objective function.] You can assume that the costs of elevation are all up front (that is, in year 0).
